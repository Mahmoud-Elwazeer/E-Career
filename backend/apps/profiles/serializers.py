...
class UserProfileSerializer(serializers.ModelSerializer):
    """Full user profile serializer (backed by CareerProfile)."""

    email = serializers.EmailField(source='user.email', read_only=True)
    full_name = serializers.CharField(source='user.get_full_name', read_only=True)
    completion_percentage = serializers.SerializerMethodField()
    is_complete = serializers.SerializerMethodField()
    desired_roles = serializers.SerializerMethodField()
    desired_locations = serializers.SerializerMethodField()
    min_salary = serializers.SerializerMethodField()
    salary_currency = serializers.SerializerMethodField()
    cv_parse_status = serializers.SerializerMethodField()
    ...
    def get_desired_roles(self, obj):
        return obj.desired_roles

    def get_desired_locations(self, obj):
        return obj.desired_locations
    ...
    def get_cv_parse_status(self, obj):
        status = obj.cv_parse_status
        if status == 'completed':
            return 'done'
        return status
...
class UserProfileUpdateSerializer(serializers.Serializer):
    ...
    preferred_type = serializers.CharField(max_length=20, required=False, allow_blank=True)
    ...
    email_alerts = serializers.BooleanField(required=False)
    alert_frequency = serializers.ChoiceField(
        choices=['instant', 'daily', 'weekly'], required=False
    )
    min_match_score = serializers.FloatField(required=False, min_value=0, max_value=1)

    def update(self, instance, validated_data):
        if 'desired_roles' in validated_data:
            instance.desired_roles = validated_data['desired_roles']
        if 'desired_locations' in validated_data:
            instance.desired_locations = validated_data['desired_locations']
        if 'portfolio_url' in validated_data:
            instance.portfolio_url = validated_data['portfolio_url']
        if 'preferred_type' in validated_data:
            instance.preferred_type = validated_data['preferred_type']
        if 'open_to_remote' in validated_data:
            instance.open_to_remote = validated_data['open_to_remote']
        if 'min_salary' in validated_data:
            instance.min_salary = validated_data['min_salary']
        if 'salary_currency' in validated_data:
            instance.salary_currency = validated_data['salary_currency']
        if 'email_alerts' in validated_data:
            instance.email_alerts = validated_data['email_alerts']
        if 'alert_frequency' in validated_data:
            instance.alert_frequency = validated_data['alert_frequency']
        if 'min_match_score' in validated_data:
            instance.min_match_score = validated_data['min_match_score']
        instance.save()
        return instance
...
class CVUploadSerializer(serializers.Serializer):
    ...
    def save(self, user):
        ...
        profile, created = CareerProfile.objects.get_or_create(user=user)
