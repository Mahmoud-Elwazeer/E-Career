/**
 * Rashid Presenting Component
 * Right hand gesturing forward (palm up, showing something)
 * Used for: showing results, recommendations, tool output
 */

import { motion } from 'framer-motion';
import { RASHID_COLORS } from './colors';
import { PRESENTING_VARIANTS, PRESENTING_ANIMATION } from './animations';

interface RashidPresentingProps {
  size?: 'xs' | 'sm' | 'md' | 'lg' | 'xl';
  className?: string;
}
...
export function RashidPresenting({ size = 'md', className = '' }: RashidPresentingProps) {
  ...
  return (
    ...
    >
      ...
      <motion.g
        initial="idle"
        animate="present"
        variants={PRESENTING_VARIANTS}
        transition={PRESENTING_ANIMATION.transition}
      >
        <path
          d="M140 230 L170 200"
          stroke={RASHID_COLORS.skinBase}
        ...
        />
        {/* Hand - Palm up */}
        <ellipse cx="170" cy="200" rx="8" ry="6" fill={RASHID_COLORS.skinBase} />
        <path
          d="M170 200 L175 195 L172 190 L168 190 L165 195"
          stroke={RASHID_COLORS.skinBase}
          strokeWidth="3"
          fill="none"
          strokeLinecap="round"
