/**
 * Rashid Wave Component
 * Standing, right hand raised waving
 * Used for: greetings, first appearance, empty states
 */

import { motion } from 'framer-motion';
import { RASHID_COLORS } from './colors';
import { WAVE_VARIANTS, WAVE_ANIMATION, BLINK_VARIANTS, BLINK_ANIMATION } from './animations';

interface RashidWaveProps {
  size?: 'xs' | 'sm' | 'md' | 'lg' | 'xl';
  className?: string;
}
...
export function RashidWave({ size = 'md', className = '' }: RashidWaveProps) {
  ...
  return (
    ...
    >
      ...
      <motion.g
        ...
        transition={WAVE_ANIMATION.transition}
      >
        <path
          d="M140 230 L160 200"
          stroke={RASHID_COLORS.skinBase}
        ...
        />
        {/* Hand */}
        <circle cx="160" cy="200" r="6" fill={RASHID_COLORS.skinBase} />
      </motion.g>
    </motion.svg>
  );
}