/**
 * Rashid Thinking Component
 * Right hand on chin, looking slightly up
 * Used for: while AI is processing/generating
 */

import { motion } from 'framer-motion';
import { RASHID_COLORS } from './colors';
import { THINKING_VARIANTS, THINKING_ANIMATION, BLINK_VARIANTS, BLINK_ANIMATION } from './animations';

interface RashidThinkingProps {
  size?: 'xs' | 'sm' | 'md' | 'lg' | 'xl';
  className?: string;
}
...
export function RashidThinking({ size = 'md', className = '' }: RashidThinkingProps) {
  ...
  return (
    ...
    >
      ...
      {/* Eyes - Looking up */}
      <g>
        <ellipse cx="90" cy="145" rx="5" ry="4" fill={RASHID_COLORS.eyesBrown} />
        <ellipse cx="110" cy="145" rx="5" ry="4" fill={RASHID_COLORS.eyesBrown} />
      </g>
      
      {/* Eyebrows - Raised */}
      <path
        d="M85 135 Q90 130 95 135"
        stroke={RASHID_COLORS.hairDark}
        strokeWidth="1.5"
        fill="none"
        strokeLinecap="round"
      ...
      <motion.g
        ...
        transition={THINKING_ANIMATION.transition}
      >
        <path
          d="M140 230 L150 190"
          stroke={RASHID_COLORS.skinBase}
        ...
        />
        {/* Hand on chin */}
        <circle cx="150" cy="190" r="6" fill={RASHID_COLORS.skinBase} />
